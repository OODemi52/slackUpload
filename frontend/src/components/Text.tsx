...
const Text = () => {
    const [uppy, setUppy] = useState<Uppy<Record<string, unknown>, Record<string, unknown>> | null>(null);
    ...
    useEffect(() => {
        ...
        uppyInstance.on('upload-success', (file, response) => handleUploadSuccess(file, response));
        uppyInstance.on('upload-error', (file, error, response) => handleUploadError(file, error, response));
