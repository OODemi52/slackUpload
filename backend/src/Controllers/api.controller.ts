...
import {
  readUser,
  writeUploadedFileReference,
  readAllUploadedFileReferencesBySession,
  paginateSlackPrivateUrls,
  anonymizeUploadedFileReferences,
} from "../Utils/db.util";
...
export const getChannels = async (
  ...
  if (process.env.NODE_ENV === "development") {
    slackAccessToken = process.env.CC_SLACK_TOKEN as string;
  } else {
    if (!request.userId) {
    ...
    slackAccessToken = await getParameterValue(
      `SLA_IDAU${user.userData.authedUser?.id}IDT${user.userData.team?.id}`,
    );
...
export const addBotToChannel = async (
  request: express.Request,
  response: express.Response,
) => {
  let slackAccessToken: string;
...
export const getImagesUrls = async (
  ...
  try {
    ...
    const { page = "1", limit = "16" } = request.query as {
      page: string;
      limit: string;
    };
    ...
    const fileReferences = await paginateSlackPrivateUrls(
      userId as string,
      pageNumber,
      limitNumber,
    );
    ...
    const imageUrls = fileReferences
      .filter(
        (fileReference) =>
          fileReference.slackPrivateFileURL && fileReference.slackFileID,
      )
      .map((fileReference) => {
        const proxyUrl = `${process.env.SERVER_PROTOCOL}://${process.env.SERVER_HOST}/api/getImagesProxy?fileID=${fileReference.slackFileID}`;
...
export const getImagesProxy = async (
  request: express.Request,
  response: express.Response,
) => {
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      if (!slackAccessToken) {
        return response
          .status(500)
          .send("Failed to retrieve Slack access token");
      }
    }

    const { fileID } = request.query;
    if (!fileID) {
      return response.status(400).send("fileID is required");
    }

    console.log("Getting info for file:", fileID);
    console.log("Slackktoken:", slackAccessToken);

    // Step 1: Fetch file info from Slack
    const infoRes = await axios.get("https://slack.com/api/files.info", {
      headers: {
    ...
    if (!infoRes.data.ok) {
      console.error("Slack files.info failed:", fileID, infoRes.data);
      return response.status(500).send("Slack files.info failed");
    }

    const downloadUrl = infoRes.data.file?.url_private_download;
    if (!downloadUrl) {
      return response.status(500).send("No download URL found in file info");
    }

    // Step 2: Download the actual file using the private download URL
    ...
    if (!contentType || contentType.includes("text/html")) {
      console.error("Slack returned HTML instead of the actual file.");
      return response.status(500).send("Invalid file response from Slack.");
    }
    ...
    console.error("Error in getImagesProxy:", error);
    response.status(500).send("Error fetching file from Slack");
  }
};
...
export const uploadProgress = async (
  ...
  const sendProgress = (progress: number) => {
    ...
    response.write(
      `data: ${JSON.stringify({ type: progress < 100.0 ? "progress" : "complete", progress })}\n\n`,
    );
  };
...
export const finalizeUpload = async (
  ...
  const getUploadJsonDataBySessionId = async (
    sessionId: string,
  ): Promise<TusUploadMetadataJsonFile[]> => {
    ...
    try {
      ...
      console.error(
        `Error reading JSON files for session ID ${sessionId}:`,
        error,
      );
      throw error;
    }
  ...
  try {
    ...
    const slackbot = new SlackBot(
      sessionFilesMetadata[0].metadata.channel,
      slackAccessToken,
    );
    ...
    const filesToUpload =
      await readAllUploadedFileReferencesBySession(sessionID);

    await slackbot.batchAndUploadFiles(
      filesToUpload,
      userID,
      sessionID,
      parseInt(sessionFilesMetadata[0].metadata.messageBatchSize),
      sessionFilesMetadata[0].metadata.comment ?? "",
      (progress) => {
  ...
  try {
    ...
    switch (files[0].deleteFlag) {
      case "app":
        ...
        response.status(200).json({
          message: `${deletedFromApp?.toString()} files deleted from Slackshots successfully.`,
        });
        break;
      case "both":
        ...
        response.status(200).json({
          message: `${deletedFromBoth?.toString()} files deleted from both Slack and Slackshots successfully.`,
        });
        break;
      default:
...
export const downloadFiles = async (
  ...
  try {
    ...
    response.setHeader(
      "Content-Disposition",
      `attachment; filename=slack_files.zip`,
    );
    ...
    for (const file of files) {
      try {
        ...
        const fileResponse = await axios.get(
          decodeURIComponent(file.url.toString()),
          {
            responseType: "stream",
            headers: {
              Authorization: `Bearer ${slackAccessToken}`,
